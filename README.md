...
- your InfluxDB instance **Host URL**, your Cloud Serverless region URL `https://us-east-1-1.aws.cloud2.influxdata.com/`.
...
1. In your terminal, create a module for your project:
...
In a Go file, import the `influxdb3` package to use it in your code:
...
Call the `influxdb3.New(config influxdb3.ClientConfig)` function with a `ClientConfig` struct that contains your [credentials](#influxdb-v3-credentials):
...
Call the `influxdb3.NewFromConnectionString(connectionString string)` function with a connection string that contains your credentials in URL format:
...
You can use Go's `defer` to close the client and raise any errors before your function exits:
...
You can provide data as "raw" line protocol:
...
You can build data as a `Point` and let `influxdb3` convert it to line protocol:
...
If the server rejects any lines in a batch, the client returns a standard API error with line-level details.
...
`influxdb3` provides an iterator for processing data rows:
...
To query with InfluxQL, call the `Query()` function and specify the `influxdb3.WithQueryType(influxdb3.InfluxQL)` option:
...
5. Run the Go file, in your terminal:

   ```sh
   go run ./Basic/basic.go
   ```
